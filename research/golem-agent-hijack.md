...
2. **Gate every fund-moving action.** `approve`/`transfer`/`send` must pass a confirmation hook, spender allowlist, or human-in-the-loop — symmetrically. The guardrail asymmetry (transfer guarded, approve not) is the specific bug to close.
